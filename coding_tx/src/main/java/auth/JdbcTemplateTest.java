...
//        dataSourceSSO.setUrl("jdbc:pivotal:greenplum://172.16.101.19:5432;DatabaseName=any_est_lw");
    ...
    @Test
    public void insertGp() {
        JdbcTemplate jrxDb19 = JdbcTemplateUtils.getJrxDb19();
        TestDataSync build = TestDataSync.builder().key_pry(1).name("测试").time2(LocalDateTime.now()).timeStampTest(LocalDateTime.now()).build();
        Field[] fields = TestDataSync.class.getDeclaredFields();
        ...
        String insertSql = getInsertSql(cols, "ods_test_data_sync_test_maxwell_data");
        System.out.println(insertSql);
        jrxDb19.update(insertSql, vals);
    }

    @Test
    public void insertForCS() throws InterruptedException {
        JdbcTemplate jrxDbMysql = JdbcTemplateUtils.getLocalMysqlGp();

        for(int i=0;i<1000;i++){
            TestDataSync build = TestDataSync.builder().name("测试").time2(LocalDateTime.now()).timeStampTest(LocalDateTime.now()).build();
            insertMysql(jrxDbMysql,build);
            Thread.sleep(1000);
        }
    }

    public void insertMysql(   JdbcTemplate jrxDbMysql ,TestDataSync build ) {
        ...
        String insertSql = getInsertSql(cols, "test_maxwell_data");
        System.out.println(insertSql);
        jrxDbMysql.update(insertSql, vals);
    }
